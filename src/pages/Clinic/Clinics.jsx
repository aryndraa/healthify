        ...
        <>
            <section className="max-h-fit  mb-4 relative">
                <div className="w-full flex  gap-2 items-center mb-4">
                    <SearchBar placeholder="Saerch clinics near you!" onSearch={setSearch} />
                                ...
                                <div className="tags mt-4 flex gap-3 flex-wrap">
                                    {typesValue.map((tag, index) => (
                                        <button
                                            ...
                                            className={`border border-[#213170] rounded-full px-4 py-2 transition-colors duration-300 ${typeActive.includes(tag) ? 'bg-[#213170] text-white' : ''}`}
                                        >
                                            {tag}
                                        </button>
                                    ))}
                    ...
                    <div className="my-4 border rounded-2xl items-center gap-4 p-8 flex bg-white md:flex-row flex-col md:rounded-full">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {typeActive.map((tag, index) => (
                                <span key={index} className="px-4 py-2 bg-[#213170] text-white rounded-full">
                                    {tag}
                                </span>
                            ))}
                ...
                {(search && search.length > 0) || type.length > 0 ? (
                    ''
                ) : (
                    <Banner
                        image={clinic}
    ...
    return (
        <div className="flex flex-col lg:flex-row-reverse lg:items-stretch gap-4 mb-4">
            <div className="hidden lg:block bg-transparent">
                <img src={image} alt="" className="w-full lg:w-[28rem] 2xl:h-[400px] 2xl:w-full object-cover lg:rounded-[60px] bg rounded-lg h-full" />
            </div>
            <div className="bg-white gap-2 lg:gap-0 flex flex-col justify-between flex-1 min-h-full rounded-xl lg:rounded-xxl p-6 lg:py-12 lg:p-14 2xl:p-14">
                <h1 className="font-semibold  text-xl lg:text-4xl 2xl:text-5xl lg:leading-[1.3] 2xl:leading-[1.2]">{title}</h1>
                <p className="text-sm 2xl:text-xl">{text}</p>
            </div>
        </div>
    );
};
