...
export function DrugDetail() {
    ...
    if (!drug) {
        return (
            <div className="min-h-screen  mb-4">
                <h1 className="text-center text-2xl font-bold">Obat tidak ditemukan</h1>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen  mb-4">
            <div className="flex flex-col xl:flex-row  justify-center xl:items-start items-center 2xl:items-start">
                <div className="w-screen pb-10 xl:pb-0 px-12 2xl:min-w-[549px] ">
                    <div className="flex justify-between items-center bg-white rounded-[40px] p-12  ">
                        <div className=" flex flex-col ">
                            <h2 className="font-semibold text--5xl">{drug.title}</h2>
                            <p className="text-gray-600 text--lg">{drug.standFor}</p>
                        </div>

                        <div>
                            <img src={drug.drugLevel} alt="Drug Level" className="w-12 mt-2" />
                        </div>
                    </div>
                    <div
                    ...
                    <div className="flex items-center justify-center bg-[#213170] text-white gap-4 py-8 px-4 rounded-[32px] " onClick={randomNavigate}>
                        <p className="font-medium text--2xl">Find In Nearest Pharmacy</p>
                ...
                <div className="bg-white rounded-[40px] p-12">
                    ...
                    <ul className="list-disc ml-5">
                        {drug.usage.map((item, index) => (
                            <li className="font-bold text--lg text-gray-500" key={index}>
                                {item}
                            </li>
                        ))}
                    </ul>
                    ...
                    <br />
                    <p className="text--xl font-semibold text-gray-500">Perhatian:</p>
                    ...
                    <br />
                    <p className="text--xl text-gray-500">{drug.additionalInfo}</p>
                </div>
            </div>
