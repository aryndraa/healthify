...
export function Pharmacies() {
    ...
    const types = ['General Medical Clinics', 'Ophthalmology Clinics', 'Dental Clinics', 'Dermatology Clinics', 'ENT Clinics', 'Physiotherapy Clinics'];
        ...
        <>
            <section className="max-h-fit relative  mb-4">
                <div className="w-full flex  gap-2 items-center mb-4">
                    <SearchBar placeholder="Search nearby pharmacy" onSearch={setSearch} />
                                ...
                                <div className="tags mt-4 flex gap-3 flex-wrap">
                                    {types.map((tag, index) => (
                                        <button
                                            ...
                                            className={`border border-[#213170] rounded-full px-4 py-2 transition-colors duration-300 ${tagsActive.includes(tag) ? 'bg-[#213170] text-white' : ''}`}
                                        >
                                            {tag}
                                        </button>
                                    ))}
