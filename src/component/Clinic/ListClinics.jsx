...
import { pharmacies } from '../../pages/Pharmacy/pharmachyData.js';

export const ListClinics = ({ isLoading }) => {
    const { type, facility, search, setSearch } = useClinic();
    const [filteredData, setFilteredData] = useState(clinics);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2  lg:gap-4 rounded-xl">
            {filteredData.map(item => (
                <PharmacyClinicCard key={item.id} img={item.images[0]} name={item.name} type={item.type} id={item.id} loading={isLoading} />
            ))}
        </div>
    );
};
