...
export const PharmacyClinicCard = ({ img, time, name, type, id, loading }) => {
    return (
        <Link to={`${id}`} className="relative w-full  2xl:w-full lg:h-[320px] xl:h-[320px] 2xl:h-[450px]">
            <img src={img} alt="" className="hidden lg:block w-full h-[240px] lg:h-[320px] 2xl:h-[450px] object-cover rounded-xl lg:rounded-xxl " />
            <div className="lg:absolute bottom-0 right-0 left-0 lg:m-2 h-full lg:h-fit bg-white  lg:rounded-xxl p-5 rounded-xl border-b lg:p-5  2xl:p-6 ">
                ...
                <div className="flex lg:items-start gap-4 lg:gap-2  justify-between items-center">
                    <div>
                        <h3 className=" lg:text-base 2xl:text-2xl font-semibold">{name}</h3>
