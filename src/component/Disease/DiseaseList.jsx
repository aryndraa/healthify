...
export const DiseaseList = ({ loading }) => {
    ...
    useEffect(() => {
        ...
        if (letterSelect && letterSelect !== 'all' && search.length === 0) {
            filtered = filtered.filter(item => item.name.toLowerCase().startsWith(letterSelect.toLowerCase()));
            setFilteredData(filtered);
        }
    }, [search, letterSelect]);
        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-2   gap-4 mb-8 w-full">
                {filteredData.map((item, index) => (
                    <DiseaseCard key={index} id={item.id} name={item.name} deskripsi={item.definisi.substring(0, 140)} gejala={item.gejala.find(g => g.name === 'umum').list} loading={loading} />
                ))}
            </div>
        );
    }
            ...
            <div>
                <div className="grid grid-cols-1 md:grid-cols-2  gap-4 mb-8 w-full">
                    {currentData.map((item, index) => (
                        <DiseaseCard key={index} id={item.id} name={item.name} deskripsi={item.definisi.substring(0, 140)} gejala={item.gejala.find(g => g.name === 'umum').list} />
                    ))}
                </div>

                <div className="flex justify-center gap-4">
                    <button onClick={prevPage} disabled={currentPage === 1} className="px-4 py-2 bg-secondary text-white disabled:opacity-50 rounded-xxl">
                        Prev
                    </button>
                    <span className="px-2 py-2 text-lg font-medium">
                        Page {currentPage} of {totalPages}
                    </span>
                    <button onClick={nextPage} disabled={currentPage === totalPages} className="px-4 py-2 bg-secondary text-white disabled:opacity-50 rounded-xxl">
                        Next
                    </button>
                </div>
            </div>
