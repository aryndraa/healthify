...
export const ListWords = ({ clearTrigger, setClearTrigger }) => {
    ...
    return (
        <div>
            ...
            {/* Mobile View Modal */}
            <button className="lg:hidden pl-6 p-2 w-full flex justify-between gap-4 items-center bg-white font-semibold rounded-full" onClick={() => setIsModalOpen(true)}>
                Select : {letterSelect}
                <span className="p-3 text-white text-xl bg-secondary rounded-full">
                    <IoMenu />
                </span>
            </button>
                    ...
                    <div className="bg-white z-50 p-6 rounded-lg shadow-lg w-[90%] max-w-md">
                        ...
                        <div className="grid grid-cols-6 gap-2">
                            <button
                                ...
                                className={`w-[40px] h-[40px] rounded-full ${letterSelect === 'all' ? 'bg-secondary text-white' : 'bg-gray-200'}`}
                            >
                                All
                            </button>
                            {letters.map(letter => (
                                <button
                                    ...
                                    className={`w-[40px] h-[40px] rounded-full ${letterSelect === letter ? 'bg-secondary text-white' : 'bg-gray-200'}`}
                                >
                                    {letter}
                                </button>
                            ))}
                        ...
                        <div className="flex justify-end">
                            <button className="mt-4 px-4 py-2 bg-gray-800 text-white  rounded-full" onClick={() => setIsModalOpen(false)}>
                                Close
                            </button>
                        </div>
                    </div>
